...
def run(args_list=None):
    ...
    parser.add_argument("--output", type=str, default="./verbalizations_enriched_fewshot.json")
    ...
    for item in tqdm(data):
        ...
        mapper_prompt = str(item.get("verbalization_mapper", "")).strip()
        # task_prompt = f"{mapper_prompt}\n\nExamples"
        task_prompt = f"\n\nExamples"
        for instance in training_instances[:3]:
            task_prompt += f"\nInput:\n{instance['input']}\n\nOutput:\n{instance['output']}"
        task_prompt += "\nPrediction:"
        item['verbalization_fewshot'] = task_prompt
        ...
        item["fewshot_verbalization_rougel"] = rouge_results["rougeL"]
