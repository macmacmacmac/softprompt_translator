...
def run(args_list=None):
    ...
    prompt_tuning_config = PromptTuningConfig(
        ...
        # prompt_tuning_init_text="1 2 3 4 5 6 7 8",
    ...
    print(f"wrapped peft dtype: {soft_prompt_model.dtype}")
    ...
    with torch.no_grad():
        generated = soft_prompt_model.generate(
            **inputs,
            do_sample=False,
            max_new_tokens=10,
            pad_token_id=tokenizer.eos_token_id
        )
        logits = soft_prompt_model(**inputs).logits
    ...
    print(f"--output shape {generated[0].shape}")
    print("---Logits: \n",logits)

    softprompt_embeddings = ([p for p in soft_prompt_model.parameters() if p.requires_grad][0]).unsqueeze(0)
    print(f"extracted softprompt dtype: {softprompt_embeddings.dtype}")
    base_model = soft_prompt_model.base_model.to(DTYPE)
    ...
    print(f"\n\nELEMENT WISE COMPARISON OVER LOGITS: {torch.equal(logits,cust_logits)}")
